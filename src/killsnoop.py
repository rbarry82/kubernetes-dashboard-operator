...
parser.add_argument("-x", "--failed", action="store_true", help="only show failed kill syscalls")
parser.add_argument("-p", "--pid", help="trace this PID only")
parser.add_argument("-s", "--signal", help="trace this signal only")
...
"""
if args.pid:
    bpf_text = bpf_text.replace("PID_FILTER", "if (pid != %s) { return 0; }" % args.pid)
else:
    bpf_text = bpf_text.replace("PID_FILTER", "")
if args.signal:
    bpf_text = bpf_text.replace("SIGNAL_FILTER", "if (sig != %s) { return 0; }" % args.signal)
else:
